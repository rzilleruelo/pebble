...
abstract class ReferenceIterator extends CompressionIterator {
    ...
    private final LongIterator referenceListIterator;
     ...
     * @throws java.io.IOException in case there is an exception reading from the input stream when initializing the
     *                     specific reference iterator.
     */
    public abstract LongIterator getReferenceListIterator(
        final int listIndex,
        final InputBitStream inputBitStream
    ) throws IOException;
    ...
    private long getNextReferenceListMatchingElement() throws IOException {
        if (remainingBlockElements > 0) {
            if (currentBit) {
                remainingBlockElements--;
                return referenceListIterator.nextLong();
            }
