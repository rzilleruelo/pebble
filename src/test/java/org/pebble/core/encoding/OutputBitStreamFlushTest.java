...
public class OutputBitStreamFlushTest {
    ...
    @Test
    public void whenBufferIsEmptyItShouldNotFlushAnything() throws IOException {
        final ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
        final OutputBitStream outputBitStream = new OutputBitStream(outputStream);
        final int expectedWrittenBytes = 0;

        outputBitStream.flush();

        assertEquals(expectedWrittenBytes, outputStream.size());
    }

    @Test
    public void whenFlushingItShouldClearBufferContent() throws IOException {
        final ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
        final OutputBitStream outputBitStream = new OutputBitStream(outputStream);
        final int x = 6;
        final int size = 3;
        final int expectedWrittenBytes = 1;
        outputBitStream.writeInBuffer(x, size);
